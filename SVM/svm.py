...
def rbf(x_1, x_2, sigma=1.):
    ...
    Returns:
        ...
        torch.tensor of type torch.float32 shaped `(#samples_1, #samples_1)`
    '''
    distances = torch.exp(- (torch.sum(x_1.square(), dim=1, keepdim=True) - 2 * torch.mm(x_1, torch.t(x_2)) + torch.sum(x_2.square(), dim=1)) / 2)
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.maximum(torch.Tensor([0]), 1 - scores[None, :] * labels[None, :])
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        Returns:
            kernel function values for all pairs of samples from x_1 and x_2
            torch.tensor shaped `(#samples_1, #samples_1)` of type torch.float32
        '''
        return (x_1 @ x_2.T).type(torch.float32)
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = (k_batch @ self.betas - self.bias).flatten() # get the matrix product using SVM parameters: self.betas and self.bias
